...
class Player07Gold600s33d84 {
    ...
    static class Ship {
    	...
    	public Ship(int id) {
    		...
    		this.next2CP = Pos.POS0;
    		this.goMiddle = false;
    		this.cntMiddle = 0;
    	}
    	public void update(Pos newPos, Pos newV, int newAngle, int newCpID, Pos newNextCP, Pos newNext2CP) {
    		if (cpID != newCpID) {
    			cntCPs++;
    		}
    		cpID = newCpID;
...
//                System.err.println("NCP: "+nextCheckPointId);
                ships[i].update(new Pos(x,y), new Pos(vx,vy), angle, nextCheckPointId, 
                		checkpoints.get(nextCheckPointId),
                		checkpoints.get((nextCheckPointId+1)%checkpointCount));
            }
            for (int i = 0; i < 2; i++) {
                Pos pos = ships[i].pos;
                Pos move = ships[i].v;
